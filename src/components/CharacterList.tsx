...
const CharacterList = memo(({ data, scrollToTop }: CharacterListProps): ReactElement => {
  const { selectedCharacter, setSelectedCharacter, setEpisodes } = useContext(CharacterContext);
    ...
    <div className="space-y-3">
      {data.map((character) => (
        ...
        >
          ...
          <div className='flex-1 text-center truncate' title={character.species}>
            {character.species}
          </div>
          ...
          <div className='flex-1 text-center truncate' title={character.location.name}>
