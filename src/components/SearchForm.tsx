...
export default function SearchForm({
	...
	const getPostsByCharacterParams = useCallback(
		async (params: CharacterFilter & { page?: number }) => {
			setLoading(true)
			try {
	...
	const getPostsByEpisode = useCallback(async (episodeId: string) => {
		...
		try {
			...
			const characters = await RickAndMortyAPI.getMultipleCharacters(
				characterIds
			)
	...
	return (
		<div className='border border-white rounded-2xl py-2 sm:py-4 px-4 sm:px-8'>
			...
			/>
			{loading && <div className='text-center  py-8 text-3xl'>Загрузка...</div>}
			{error ? (
				<div className='text-center py-8'>
					...
					/>
					<p className=' text-5xl mt-4'>Ничего не найдено</p>
				</div>
			) : (
